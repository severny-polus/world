...
{
	if (region.a0.ak === region.bd.ak)
	{
		return 'on line ' + region.a0.ak;
	}
	return 'on lines ' + region.a0.ak + ' through ' + region.bd.ak;
}
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { cJ: 'hidden', cq: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { cJ: 'mozHidden', cq: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { cJ: 'msHidden', cq: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { cJ: 'webkitHidden', cq: 'webkitvisibilitychange' }
		: { cJ: 'hidden', cq: 'visibilitychange' };
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		$elm$core$Maybe$isJust(request.bR) && _Http_track(router, xhr, request.bR.a);

		try {
			xhr.open(request.cZ, request.aF, true);
		} catch (e) {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {bg: fragment, bl: host, bA: path, bC: port_, bG: protocol, bH: query};
	});
...
var $author$project$Main$config = {
	s: {aR: 'ne_110m_lakes', aS: 'ne_110m_land_antarctica', aT: 'ne_110m_land_no_antarctica', aZ: 'ne_110m_rivers_lake_centerlines'},
	bD: 'https://raw.githubusercontent.com/severny-polus/world/master'
};
...
var $author$project$Main$geodataResource = function (name) {
	return $author$project$Main$config.bD + ('/geodata/' + (name + '.min.geo.json'));
};
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{b9: false, cg: r.cg, ay: r.ay, bi: r.bi, cZ: r.cZ, dK: r.dK, bR: r.bR, aF: r.aF}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{cg: $elm$http$Http$emptyBody, ay: r.ay, bi: _List_Nil, cZ: 'GET', dK: $elm$core$Maybe$Nothing, bR: $elm$core$Maybe$Nothing, aF: r.aF});
};
var $author$project$Geodata$init = {aR: _List_Nil, aS: _List_Nil, aT: _List_Nil, aZ: _List_Nil};
...
var $author$project$Projection$initAngle = $elm$core$Basics$pi + $elm$core$Basics$degrees(56);
var $author$project$Projection$init = _Utils_Tuple2(
	{
		ah: $author$project$Projection$initAngle,
		aK: 0,
		ai: 0,
...
var $author$project$Math$Polygon = F2(
	function (exterior, interiors) {
		return {cC: exterior, cV: interiors};
	});
	...
	var requests = _List_fromArray(
		[
			$elm$http$Http$get(
			{
				...
				aF: $author$project$Main$geodataResource($author$project$Main$config.s.aT)
			}),
			$elm$http$Http$get(
			{
				ay: A2(
					...
					$author$project$GeoJson$polygons),
				aF: $author$project$Main$geodataResource($author$project$Main$config.s.aS)
			}),
			$elm$http$Http$get(
			{
				ay: A2(
					...
					$author$project$GeoJson$polygons),
				aF: $author$project$Main$geodataResource($author$project$Main$config.s.aR)
			}),
			$elm$http$Http$get(
			{
				ay: A2(
				...
				aF: $author$project$Main$geodataResource($author$project$Main$config.s.aZ)
			})
		]);
	...
	return _Utils_Tuple2(
		{
			...
			ab: projection,
			al: $elm$core$List$length(requests)
		},
...
var $elm$browser$Browser$AnimationManager$State = F3(
	function (subs, request, oldTime) {
		return {aW: oldTime, bK: request, bP: subs};
	});
...
var $author$project$Main$subscriptions = function (model) {
	return A2(
		...
		$author$project$Projection$subscriptions(model.ab));
};
...
var $author$project$Projection$maxShowParam = 1;
var $author$project$Projection$maxShowParamAcceleration = 2;
					...
					function () {
						if (!maybeElement.$) {
							...
							return _Utils_update(
								projection,
								{
									aD: _Utils_Tuple2(element.bc.dS, element.bc.cH)
								});
						} else {
							return projection;
						}
				...
				return _Utils_Tuple2(
					_Utils_update(
						projection,
						{
							...
							U: $author$project$Projection$maxShowParamAcceleration
						}),
					$elm$core$Platform$Cmd$none);
			default:
				var dtMs = msg.a;
				...
				return _Utils_Tuple2(
					function () {
						if (!projection.U) {
							return _Utils_update(
								projection,
								{ah: projection.ah + (projection.ai * dt), ai: projection.ai + (projection.aK * dt)});
						} else {
							var showParam = projection.J + (projection.ac * dt);
							return (_Utils_cmp(showParam, $author$project$Projection$maxShowParam) < 0) ? _Utils_update(
								projection,
								{
									J: showParam,
									U: (showParam < 0.5) ? $author$project$Projection$maxShowParamAcceleration : (-$author$project$Projection$maxShowParamAcceleration),
									ac: projection.ac + (projection.U * dt)
								}) : _Utils_update(
								projection,
								{J: $author$project$Projection$maxShowParam, U: 0, ac: 0});
						}
					}(),
					$elm$core$Platform$Cmd$none);
		}
				...
				if (!result.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								aM: $elm$core$Maybe$Just(error)
							}),
						$elm$core$Platform$Cmd$none);
				}
			} else {
				var projectionMsg = msg.a;
				var _v2 = A2($author$project$Projection$update, projectionMsg, model.ab);
...
var $mdgriffith$elm_ui$Internal$Style$classes = {b$: 'a', aH: 'atv', b1: 'ab', b2: 'cx', b3: 'cy', b4: 'acb', b5: 'accx', b6: 'accy', b7: 'acr', a6: 'al', a7: 'ar', b8: 'at', aI: 'ah', aJ: 'av', ca: 's', ce: 'bh', cf: 'b', ch: 'w7', cj: 'bd', ck: 'bdt', ar: 'bn', cl: 'bs', as: 'cpe', cr: 'cp', cs: 'cpx', ct: 'cpy', E: 'c', au: 'ctr', av: 'cb', aw: 'ccx', F: 'ccy', aj: 'cl', ax: 'cr', cv: 'ct', cw: 'cptr', cx: 'ctxt', cE: 'fcs', bf: 'focus-within', cF: 'fs', cG: 'g', aO: 'hbh', aP: 'hc', bj: 'he', aQ: 'hf', bk: 'hfp', cK: 'hv', cM: 'ic', cO: 'fr', aA: 'lbl', cQ: 'iml', cR: 'imlf', cS: 'imlp', cT: 'implw', cU: 'it', cW: 'i', br: 'lnk', aa: 'nb', bu: 'notxt', c0: 'ol', c2: 'or', R: 'oq', c6: 'oh', by: 'pg', bz: 'p', c7: 'ppe', db: 'ui', v: 'r', dd: 'sb', de: 'sbx', df: 'sby', dg: 'sbt', dj: 'e', dk: 'cap', dl: 'sev', dt: 'sk', p: 't', dw: 'tc', dx: 'w8', dy: 'w2', dz: 'w9', dA: 'tj', aE: 'tja', dB: 'tl', dC: 'w3', dD: 'w5', dE: 'w4', dF: 'tr', dG: 'w6', dH: 'w1', dI: 'tun', bS: 'ts', V: 'clr', dP: 'u', a2: 'wc', bW: 'we', a3: 'wf', bX: 'wfp', a4: 'wrp'};
var $mdgriffith$elm_ui$Internal$Model$columnClass = $mdgriffith$elm_ui$Internal$Style$classes.ca + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.E);
var $mdgriffith$elm_ui$Internal$Model$gridClass = $mdgriffith$elm_ui$Internal$Style$classes.ca + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.cG);
...
var $mdgriffith$elm_ui$Internal$Model$rowClass = $mdgriffith$elm_ui$Internal$Style$classes.ca + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.v);
var $mdgriffith$elm_ui$Internal$Model$singleClass = $mdgriffith$elm_ui$Internal$Style$classes.ca + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.dj);
	...
	switch (style.$) {
		...
		case 8:
			...
			return 'grid-rows-' + (A2(
				...
				$elm$core$String$join,
				'-',
				A2($elm$core$List$map, $mdgriffith$elm_ui$Internal$Model$lengthClassName, template.y)) + ('-space-x-' + ($mdgriffith$elm_ui$Internal$Model$lengthClassName(template.dm.a) + ('-space-y-' + $mdgriffith$elm_ui$Internal$Model$lengthClassName(template.dm.b)))))));
		case 9:
			var pos = style.a;
			return 'gp grid-pos-' + ($elm$core$String$fromInt(pos.v) + ('-' + ($elm$core$String$fromInt(pos.cu) + ('-' + ($elm$core$String$fromInt(pos.dS) + ('-' + $elm$core$String$fromInt(pos.cH)))))));
		case 11:
...
var $mdgriffith$elm_ui$Internal$Model$renderFocusStyle = function (focus) {
	return _List_fromArray(
		[
			...
			$mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.bf) + ':focus-within',
			A2(
				$elm$core$List$filterMap,
				$elm$core$Basics$identity,
				_List_fromArray(
			...
			A2(
			$mdgriffith$elm_ui$Internal$Model$Style,
			($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.ca) + ':focus .focusable, ') + (($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.ca) + '.focusable:focus, ') + ('.ui-slide-bar:focus + ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.ca) + ' .focusable-thumb'))),
			A2(
				$elm$core$List$filterMap,
				$elm$core$Basics$identity,
				_List_fromArray(
		...
		_List_fromArray(
			[
				...
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.cK, $mdgriffith$elm_ui$Internal$Style$classes.V)) + ':hover',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.cK, $mdgriffith$elm_ui$Internal$Style$classes.R)) + ':hover',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '1')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.cE, $mdgriffith$elm_ui$Internal$Style$classes.V)) + ':focus',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.aH, $mdgriffith$elm_ui$Internal$Style$classes.V)) + ':active',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '0')
					])),
				...
				$mdgriffith$elm_ui$Internal$Style$Descriptor,
				$mdgriffith$elm_ui$Internal$Style$dot(
					_Utils_ap($mdgriffith$elm_ui$Internal$Style$classes.aH, $mdgriffith$elm_ui$Internal$Style$classes.R)) + ':active',
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Internal$Style$Prop, 'opacity', '1')
					])),
...
var $mdgriffith$elm_ui$Internal$Style$overrides = '@media screen and (-ms-high-contrast: active), (-ms-high-contrast: none) {' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.ca) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.v) + (' > ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.ca) + (' { flex-basis: auto !important; } ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.ca) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.v) + (' > ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.ca) + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.au) + (' { flex-basis: auto !important; }}' + ($mdgriffith$elm_ui$Internal$Style$inputTextReset + ($mdgriffith$elm_ui$Internal$Style$sliderReset + ($mdgriffith$elm_ui$Internal$Style$trackReset + ($mdgriffith$elm_ui$Internal$Style$thumbReset + $mdgriffith$elm_ui$Internal$Style$explainer)))))))))))))));
...
var $mdgriffith$elm_ui$Internal$Style$emptyIntermediate = F2(
	function (selector, closing) {
		return {at: closing, i: _List_Nil, I: _List_Nil, w: selector};
	});
					...
					case 3:
						...
						return _Utils_update(
							rendered,
							{
								i: A2(
									$elm$core$List$cons,
									{at: '\n}', i: _List_Nil, I: props, w: '@supports (' + (prop + (':' + (value + (') {' + parent.w))))},
									rendered.i)
							});
					case 5:
						var selector = rule.a;
...
var $mdgriffith$elm_ui$Internal$Style$renderCompact = function (styleClasses) {
	...
	var renderClass = function (rule) {
		...
		if (!_v2.b) {
			...
			return rule.w + ('{' + (renderValues(rule.I) + (rule.at + '}')));
		}
	};
				...
				case 0:
					...
					return _List_fromArray(
						[
							...
							('.ui-slide-bar:focus + ' + ($mdgriffith$elm_ui$Internal$Style$dot($mdgriffith$elm_ui$Internal$Style$classes.ca) + (' .focusable-thumb' + (selector + '-fs {')))) + (renderedProps + '\n}')
						]);
				default:
					return _List_fromArray(
						[
	...
	function (options, rule, maybePseudo) {
		switch (rule.$) {
			...
			case 8:
				...
				var xSpacing = toGridLength(template.dm.a);
				var ySpacing = toGridLength(template.dm.b);
				var rows = function (x) {
				...
				var msColumns = function (x) {
					...
					A2(
						...
						ySpacing,
						A2($elm$core$List$map, toGridLength, template.y)));
				var gapY = 'grid-row-gap:' + (toGridLength(template.dm.b) + ';');
				var gapX = 'grid-column-gap:' + (toGridLength(template.dm.a) + ';');
			...
			case 9:
				...
				var modernPosition = A2(
					...
					_List_fromArray(
						[
							'grid-row: ' + ($elm$core$String$fromInt(position.v) + (' / ' + ($elm$core$String$fromInt(position.v + position.cH) + ';'))),
							'grid-column: ' + ($elm$core$String$fromInt(position.cu) + (' / ' + ($elm$core$String$fromInt(position.cu + position.dS) + ';')))
						]));
				var _class = '.grid-pos-' + ($elm$core$String$fromInt(position.v) + ('-' + ($elm$core$String$fromInt(position.cu) + ('-' + ($elm$core$String$fromInt(position.dS) + ('-' + $elm$core$String$fromInt(position.cH)))))));
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A2(
					$mdgriffith$elm_ui$Internal$Model$bracket,
					'.' + (name + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.dk + ('> .' + ($mdgriffith$elm_ui$Internal$Style$classes.p + (', .' + (name + (' .' + ($mdgriffith$elm_ui$Internal$Style$classes.dk + (' > .' + $mdgriffith$elm_ui$Internal$Style$classes.p)))))))))),
					_List_fromArray(
						[
...
var $mdgriffith$elm_ui$Internal$Model$adjust = F3(
	function (size, height, vertical) {
		return {cH: height / size, aD: size, bU: vertical};
	});
...
var $mdgriffith$elm_ui$Internal$Model$convertAdjustment = function (adjustment) {
	var lines = _List_fromArray(
		[adjustment.cp, adjustment.cd, adjustment.cy, adjustment.cY]);
	var lineHeight = 1.5;
	...
	return {
		...
		bh: A3($mdgriffith$elm_ui$Internal$Model$adjust, fullSize, ascender - descender, fullVertical)
	};
};
					...
					return A3(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$class($mdgriffith$elm_ui$Internal$Style$classes.ca + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.dj))
									]))
							]));
			}
		}();
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.ca, $mdgriffith$elm_ui$Internal$Style$classes.dj, $mdgriffith$elm_ui$Internal$Style$classes.au, $mdgriffith$elm_ui$Internal$Style$classes.F, $mdgriffith$elm_ui$Internal$Style$classes.b7])))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.ca, $mdgriffith$elm_ui$Internal$Style$classes.dj, $mdgriffith$elm_ui$Internal$Style$classes.au, $mdgriffith$elm_ui$Internal$Style$classes.F, $mdgriffith$elm_ui$Internal$Style$classes.b5])))
						]),
					_List_fromArray(
						[html])) : html));
			case 1:
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.ca, $mdgriffith$elm_ui$Internal$Style$classes.dj, $mdgriffith$elm_ui$Internal$Style$classes.au, $mdgriffith$elm_ui$Internal$Style$classes.b6])))
						]),
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							A2(
								...
								' ',
								_List_fromArray(
									[$mdgriffith$elm_ui$Internal$Style$classes.ca, $mdgriffith$elm_ui$Internal$Style$classes.dj, $mdgriffith$elm_ui$Internal$Style$classes.au, $mdgriffith$elm_ui$Internal$Style$classes.b4])))
						]),
					_List_fromArray(
						[html])) : html));
			default:
...
var $mdgriffith$elm_ui$Internal$Model$textElementClasses = $mdgriffith$elm_ui$Internal$Style$classes.ca + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.p + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.a2 + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.aP)))));
...
var $mdgriffith$elm_ui$Internal$Model$textElementFillClasses = $mdgriffith$elm_ui$Internal$Style$classes.ca + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.p + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.a3 + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.aQ)))));
						...
						switch (location) {
							case 0:
								return A2(
									...
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.aa, $mdgriffith$elm_ui$Internal$Style$classes.dj, $mdgriffith$elm_ui$Internal$Style$classes.b$]));
							case 1:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.aa, $mdgriffith$elm_ui$Internal$Style$classes.dj, $mdgriffith$elm_ui$Internal$Style$classes.cf]));
							case 2:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.aa, $mdgriffith$elm_ui$Internal$Style$classes.dj, $mdgriffith$elm_ui$Internal$Style$classes.c2]));
							case 3:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.aa, $mdgriffith$elm_ui$Internal$Style$classes.dj, $mdgriffith$elm_ui$Internal$Style$classes.c0]));
							case 4:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.aa, $mdgriffith$elm_ui$Internal$Style$classes.dj, $mdgriffith$elm_ui$Internal$Style$classes.cO]));
							default:
								return A2(
									$elm$core$String$join,
									' ',
									_List_fromArray(
										[$mdgriffith$elm_ui$Internal$Style$classes.aa, $mdgriffith$elm_ui$Internal$Style$classes.dj, $mdgriffith$elm_ui$Internal$Style$classes.ce]));
						}
					}())
				]),
			_List_fromArray(
...
var $mdgriffith$elm_ui$Internal$Model$alignXName = function (align) {
	switch (align) {
		case 0:
			return $mdgriffith$elm_ui$Internal$Style$classes.aI + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.a6);
		case 2:
			return $mdgriffith$elm_ui$Internal$Style$classes.aI + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.a7);
		default:
			return $mdgriffith$elm_ui$Internal$Style$classes.aI + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.b2);
	}
};
var $mdgriffith$elm_ui$Internal$Model$alignYName = function (align) {
	switch (align) {
		...
		case 2:
			return $mdgriffith$elm_ui$Internal$Style$classes.aJ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.b1);
		default:
			return $mdgriffith$elm_ui$Internal$Style$classes.aJ + (' ' + $mdgriffith$elm_ui$Internal$Style$classes.b3);
	}
};
	...
	switch (h.$) {
		case 0:
			...
			return _Utils_Tuple3(
				...
				$mdgriffith$elm_ui$Internal$Style$classes.bj + (' ' + name),
				_List_fromArray(
					[
			...
			return (portion === 1) ? _Utils_Tuple3(
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Internal$Style$classes.ca + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.E + (' > ' + $mdgriffith$elm_ui$Internal$Style$dot(
			...
			return (portion === 1) ? _Utils_Tuple3(
				...
				_List_fromArray(
					[
						A3(
						$mdgriffith$elm_ui$Internal$Model$Single,
						$mdgriffith$elm_ui$Internal$Style$classes.ca + ('.' + ($mdgriffith$elm_ui$Internal$Style$classes.v + (' > ' + $mdgriffith$elm_ui$Internal$Style$dot(
						...
						if (A2($mdgriffith$elm_ui$Internal$Flag$present, $mdgriffith$elm_ui$Internal$Flag$height, has)) {
							...
							switch (height.$) {
								case 0:
									...
									var name = 'height-px-' + val;
									var $temp$classes = $mdgriffith$elm_ui$Internal$Style$classes.bj + (' ' + (name + (' ' + classes))),
										$temp$node = node,
								...
								case 2:
									...
									if (portion === 1) {
										...
										var $temp$classes = classes + (' ' + ($mdgriffith$elm_ui$Internal$Style$classes.bk + (' height-fill-' + $elm$core$String$fromInt(portion)))),
											$temp$node = node,
											$temp$has = A2(
											$mdgriffith$elm_ui$Internal$Flag$add,
											$mdgriffith$elm_ui$Internal$Flag$heightFill,
...
var $mdgriffith$elm_ui$Internal$Model$optionsToRecord = function (options) {
	...
	return andFinally(
		A3(
			$elm$core$List$foldr,
			combine,
			{cE: $elm$core$Maybe$Nothing, cK: $elm$core$Maybe$Nothing, c_: $elm$core$Maybe$Nothing},
			options));
};
			...
			A2(
				...
				$mdgriffith$elm_ui$Internal$Model$htmlClass(
					A2(
						...
						' ',
						_List_fromArray(
							[$mdgriffith$elm_ui$Internal$Style$classes.db, $mdgriffith$elm_ui$Internal$Style$classes.ca, $mdgriffith$elm_ui$Internal$Style$classes.dj]))),
...
var $author$project$Projection$r = function (theta) {
	return $elm$core$Basics$sin(theta / 2);
};
var $author$project$Projection$transform = F2(
	function (zoom, _v0) {
		var phi = _v0.a;
		var theta = _v0.b;
		return A2(
			$author$project$Projection$fromPolar,
			(zoom * $author$project$Projection$r(theta / zoom)) / $author$project$Projection$r($elm$core$Basics$pi / zoom),
			phi);
	});
...
var $author$project$Projection$view = function (projection) {
	...
	var rotate = function (_v3) {
		var phi = _v3.a;
		var theta = _v3.b;
		return _Utils_Tuple2((phi + projection.ah) + (($elm$core$Basics$pi / 3) * (1 - projection.J)), theta);
	};
	var _v0 = projection.aD;
	var w = _v0.a;
	var h = _v0.b;
	var a = projection.J * A2($elm$core$Basics$min, w, h);
	...
	var project = A2(
		...
		A2(
			...
			A2(
				$elm$core$Basics$composeR,
				$author$project$Projection$transform(projection.aG),
				scale)));
		...
		return $elm$core$List$concat(
			_List_fromArray(
				[
					...
					A2($elm$core$List$map, water, pol.cV)
				]));
	};
		...
		return $elm$core$List$concat(
			_List_fromArray(
				[
					...
					A2($elm$core$List$map, land, pol.cV)
				]));
	};
							...
							[
								...
								_List_fromArray(
									[
										...
										$elm_community$typed_svg$TypedSvg$Attributes$InPx$r((a / 2) * projection.aG),
										$elm_community$typed_svg$TypedSvg$Attributes$fill(
										$elm_community$typed_svg$TypedSvg$Types$Paint($author$project$Projection$colorLand))
									]),
								_List_Nil)
							]),
							A2($elm$core$List$concatMap, waterWater, geodata.aS),
							A2($elm$core$List$concatMap, landWater, geodata.aT),
							A2($elm$core$List$map, river, geodata.aZ),
							A2($elm$core$List$concatMap, waterLand, geodata.aR),
							_List_fromArray(
							[
								A2(
								...
								_List_fromArray(
									[
										...
										$elm_community$typed_svg$TypedSvg$Types$Paint(
											A4($avh4$elm_color$Color$rgba, 1, 1, 1, 1 - projection.J)))
									]),
								_List_Nil)
							])
						]));
		...
		A2(
			...
			$mdgriffith$elm_ui$Element$html(
				A2(
					$elm$html$Html$map,
					$author$project$Main$ProjectionMsg,
					$author$project$Projection$view(model.ab)))));
};
var $author$project$Main$main = $elm$browser$Browser$element(
	{cP: $author$project$Main$init, dv: $author$project$Main$subscriptions, dQ: $author$project$Main$update, dR: $author$project$Main$view});
